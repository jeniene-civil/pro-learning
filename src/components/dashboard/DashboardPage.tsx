  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div>
            ...
            <p className="text-brand-100 text-sm">
              {activeCourses.length > 0
                ? `You have ${activeCourses.length} active course${activeCourses.length > 1 ? "s" : ""} in progress.`
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { icon: BookOpen, value: activeCourses.length.toString(), label: "Active Courses", color: "text-blue-600 bg-blue-50" },
          { icon: Clock, value: enrollments.length.toString(), label: "Total Enrolled", color: "text-emerald-600 bg-emerald-50" },
          { icon: Award, value: completedCourses.length.toString(), label: "Completed", color: "text-amber-600 bg-amber-50" },
          { icon: BookOpen, value: activeCourses.length > 0 ? `${Math.round(activeCourses.reduce((a: number, e: any) => a + (e.progress || 0), 0) / activeCourses.length)}%` : "—", label: "Avg Progress", color: "text-purple-600 bg-purple-50" },
        ].map((stat) => (
        ...
        <div className="bg-white rounded-2xl border border-border p-6">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-center gap-2 text-gray-500">
              ...
              <span>Role: <span className="text-gray-900 font-medium">{userRole}</span></span>
            </div>
            <div className="flex items-center gap-2 text-gray-500">
          ...
          {latest ? (
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-medium text-gray-900 truncate">{latest.course.title}</h3>
                <p className="text-sm text-gray-500 mb-2">{latest.course.instructor?.name}</p>
                <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                  <div className="bg-brand rounded-full h-2" style={{ width: `${latest.progress || 0}%` }} />
                </div>
                <p className="text-xs text-gray-400">{Math.round(latest.progress || 0)}% complete</p>
              </div>
              <Link
                href={`/${locale}/courses/${latest.course.slug}/learn`}
            ...
            <div className="text-center py-8">
              ...
              <Link href={`/${locale}/courses`} className="inline-flex px-5 py-2.5 bg-brand text-white text-sm font-semibold rounded-lg hover:bg-brand-700 transition-colors">
                Browse Courses
              </Link>
            </div>
          )}
        ...
        {enrollments.length > 0 ? (
          <div className="space-y-4">
            {enrollments.map((enrollment) => (
              <Link
                key={enrollment.id}
                href={`/${locale}/courses/${enrollment.course.slug}/learn`}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">{enrollment.course.instructor?.name} · {enrollment.course.duration}</p>
                  <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2">
                    <div className="bg-brand rounded-full h-1.5" style={{ width: `${Math.round(enrollment.progress || 0)}%` }} />
                  </div>
                  <p className="text-xs text-gray-400 mt-0.5">{Math.round(enrollment.progress || 0)}% complete</p>
                </div>
              </Link>
            ))}
          </div>
